...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="queryRelativeUri" select="'MarkLogicQuery_ExternalVariables.xqy'" as="xs:string"/>
    
    <xsl:variable name="result"
                  as="item()+"
                  select="mkl-ext:marklogic-query-uri($queryRelativeUri, $config, $extVar)"/>
